...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    val legalMoves = b.legalNeighbors
    legalMoves map (m => (m._1, m._2 :: history)) toStream
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.tail.isEmpty && newNeighbors.isEmpty)
      initial
    else
      // Breadth first: search items in initial list first, then neighbors in order... keep appending to end of list
      initial.head #:: from(initial.tail #::: newNeighbors, explored union (newNeighbors map (e => e._1)).toSet)
  }
